...
This directory contains **true end-to-end tests** that validate the complete payment authorization flow across all services running in separate Docker containers with real HTTP requests and network communication.

## Overview

These E2E tests differ from integration tests by running each service in its own Docker container:
...
|---------|------------------|-----------|
| **Services** | In-process (ASGI/asyncio) | Separate Docker containers |
| **HTTP** | ASGI test client | Real HTTP over network |
| **Payment Token Service** | Mocked | Real service with encryption |
| **Worker** | In-process asyncio | Separate container process |
| **Realism** | Medium | High (production-like) |
| **Speed** | Fast (5-15s per test) | Slower (20-60s per test) |
| **Purpose** | Development/CI | Pre-deployment validation |
...
**File:** `tests/e2e/helpers/http_client.py`
...
**File:** `tests/e2e/fixtures/docker_fixtures.py`
...
- **Integration Tests:** `tests/README.md` - In-process integration tests
